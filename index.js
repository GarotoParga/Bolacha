...
function drawStars() { ctx.clearRect(0, 0, canvas.width, canvas.height); stars.forEach(star => { ctx.beginPath(); ctx.arc(star.x, star.y, star.radius, 0, Math.PI * 2); ctx.fillStyle = "#fff"; ctx.fill(); star.y += star.speed; if (star.y > canvas.height) star.y = 0 }); requestAnimationFrame(drawStars); }
drawStars();
...
function updateTime() {
  ...
  document.getElementById('timePassed').textContent = `Já se passaram: ${years} anos, ${months} meses, ${days} dias, ${hours} horas, ${minutes} minutos e ${seconds} segundos`;
}
updateTime(); setInterval(updateTime, 1000);

// ---------------- Contador infinito ----------------
const counter = document.getElementById('counter'); let value = 0n;
...
// ---------------- Barra poética ----------------
const bar = document.getElementById('bar'); let barValue = 0; let messageIndex = 0;
...
// inicia barra e contador ao fechar mensagem
